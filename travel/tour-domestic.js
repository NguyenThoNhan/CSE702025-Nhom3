// tours-domestic.js (PHIÊN BẢN SỬA LỖI)
document.addEventListener('DOMContentLoaded', () => {

    const toursContainer = document.getElementById('domestic-tours-container');
    const paginationContainer = document.getElementById('pagination');
    const sortBar = document.getElementById('sort-bar');

    // SỬA ĐIỀU KIỆN LỌC: Lấy tất cả tour không phải là "Tour Nước Ngoài"
    let allDomesticTours = Object.values(toursDB).filter(tour => 
    ...
    function displayTours() {
        if (!toursContainer) {
            console.error("Không tìm thấy container #domestic-tours-container");
            return;
        }

        let toursToDisplay = [...allDomesticTours];
        switch (currentSort) {
        ...
        if (pageTours.length === 0) {
            toursContainer.innerHTML = '<p class="empty-message">Không tìm thấy tour trong nước nào.</p>';
        } else {
    ...
    function setupPagination() {
        ...
        const pageCount = Math.ceil(allDomesticTours.length / toursPerPage);
